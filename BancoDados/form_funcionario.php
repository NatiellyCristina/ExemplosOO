<?php
	...
	//seleção dos valores que irão criar o <select> de departamento
	...
	$v = array("type"=>"text","name"=>"ID_FUNCIONARIO","placeholder"=>"ID DO FUNCIONARIO...", "value"=>$value_id_funcionario);
	$f->add_input($v);
	...
	$f->add_input($v);
	$v = array("type"=>"date","label"=>"Data_Contratação: ", "name"=>"DATA_CONTRATACAO","placeholder"=>"DATA CONTRATAÇÃO...", "value"=>$value_data_contratacao);
	$f->add_input($v);
	
	$v = array("name"=>"ID_FUNCAO","label"=>"Função", "selected"=>$value_id_funcao);
	...
	$f->add_input($v);	
	$v = array("name"=>"ID_GERENTE","label"=>"Gerente", "selected"=>$value_id_gerente);
	$f->add_select($v,$matriz_gerente);
	$v = array("name"=>"ID_DEPARTAMENTO","label"=>"Departamento", "selected"=>$value_id_departamento);
